...
abstract class TimestampBasedMongoRepository[A <: CaseClass with HasTimestamp](implicit m: Manifest[A])
  ...
  def load(filter:(String, Any), timeQuery: (String, DateTime), sort: Order = Descending) = SimpleMongoCursor(
    query(filter, (databaseTimeStampProperty, where(timeQuery))), order((databaseTimeStampProperty, sort.order))
  )
